...
server.tool("calculate_fba_cost", "按 2026 美国 FBA（非服装）规则计算海运、配置费、配送费和总费用。海运按实重与体积重较高者计费。", {
  ...inputSchema, season: z.enum(["regular", "peak"]).default("regular"), price_band: z.enum(["under_10", "10_to_50", "over_50"]).default("10_to_50"), sea_freight_template_id: z.enum(["202607-lianyu-express", "202607-lianyu-oa"]).default("202607-lianyu-express"), sea_rate_per_kg: z.number().positive().optional(), placement: z.enum(["single", "split", "optimized"]).default("single")
...
export default { async fetch(request, env, ctx) { const url = new URL(request.url); if (url.pathname === "/mcp") return createMcpHandler(buildServer())(request, env, ctx); return env.ASSETS.fetch(request); } };
